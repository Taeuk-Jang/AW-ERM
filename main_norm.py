...
from verb_classification.model_norm import *
import logging
...
def main():
    ...
    parser.add_argument('--annotation_dir', type = str, default = './verb_classification/data')
    parser.add_argument('--image_dir', type = str, default = './verb_classification/data/of500_images_resized')
    parser.add_argument('--image_size', type = int, default = 256)
    parser.add_argument('--crop_size', type = int, default = 224)
    parser.add_argument('--balanced', type = bool, default = False)
    parser.add_argument('--ratio', type = int, default = 0)
    parser.add_argument('--gender_balanced', type = bool, default = False)
    parser.add_argument('--blackout_box', type = bool, default = False)
    parser.add_argument('--blackout_face', type = bool, default = False)
    parser.add_argument('--no_image', type = bool, default = False)
...
#     args.device = 'cuda:1'
    ...
    if not args.crt_type == 'baseline':
        args.save_dir = 'inital_test/' + args.log_prefix + '_' + args.task_type + '_' + str(args.soft) + '_' + str(args.num_epochs) + '_' + str(args.lr) + '_' + str(args.lr_w) + '_' + args.train_type + '_' \
        + args.crt_type + '/' + timestr
        args.log_dir = 'inital_test/' + args.log_prefix + '_' + args.task_type + '_' + str(args.soft) + '_' + str(args.num_epochs) + '_' + str(args.lr) + '_' + str(args.lr_w) + '_' + args.train_type + '_' \
        + args.crt_type + '/' + timestr
    else:
        args.save_dir = 'inital_test/' + args.log_prefix + '_' + str(args.num_epochs) + '_' + str(args.lr) + '_' + args.crt_type + '/' + timestr
        args.log_dir = 'inital_test/' + args.log_prefix + '_' + str(args.num_epochs) + '_' + str(args.lr) + '_' + args.crt_type + '/' + timestr
...
#     logging.getLogger().addHandler(stderrLogger)
    ...
    if not args.train_type == 'baseline':
        args.optim_w = torch.optim.Adam(args.w_model.parameters(), args.lr_w, weight_decay = 1e-4)
...
#         print(cnt>10)
#         if args.dataset == 'celeba' and cnt > 10:
#             t.close()
#             break
...
#                 weight = args.w_model(features, targets.float()).detach()
                weight = args.w_model(features, targets.argmax(-1).view(-1,1).float()).detach()
            elif args.task_type == 'gender':
                weight = args.w_model(features, genders.argmax(-1).view(-1,1).float()).detach()
            #case 1
            weight = 1 + weight
            #case 2
...
#                 weight = args.w_model(features.detach(), targets.float())
                weight = args.w_model(features.detach(), targets.argmax(-1).view(-1,1).float())
            elif args.task_type == 'gender':
                weight = args.w_model(features.detach(), genders.argmax(-1).view(-1,1).float())
